/**
 * CareerForge AI — Fix Design & Navigation System
 * Resolves:
 * 1. Missing `.page-view { display: none }` causing all 7 pages to stack simultaneously
 * 2. Proper `.page-view.active { display: block !important }` with smooth fade entrance
 * 3. Default active page set to 'dashboard' (Overview) on login/unlock
 * 4. Implements `toggleThemeMode()` for light/dark switching with chart re-rendering
 * 5. Full Autumn Orchard styling for all 7 page views & components
 */
...
// 1. Ensure default data-theme is "light"
html = html.replace(/<html lang="en" data-theme="[^"]*">/, '<html lang="en" data-theme="light">');

// 2. Comprehensive CSS with proper Page View Isolation & Autumn Orchard Polish
const completeCss = `
/* ═══════════════════════════════════════════════════════════════
...
/* ── Light Mode (Primary Autumn Orchard Canvas) ── */
[data-theme="light"], :root {
  ...
  --border-glass-strong: rgba(26, 26, 24, 0.14);
  ...
  --shadow-card: 0 4px 6px -1px rgba(26, 26, 24, 0.05), 0 10px 24px -2px rgba(26, 26, 24, 0.04);
...
/* ═══════════════════════════════════════════════════════════════
   CRITICAL: PAGE VIEW NAVIGATION ISOLATION
...
.page-view.active {
  ...
  animation: pageFadeSlide 0.28s cubic-bezier(0.16, 1, 0.3, 1);
}
...
.dashboard-hero-banner {
  background: linear-gradient(135deg, var(--primary-subtle), var(--surface-sunken));
  border: 1px solid var(--primary-border);
...
.section-heading-title { font-family: var(--font-heading); font-size: 1.25rem; font-weight: 700; }
.section-heading-sub { font-size: 0.82rem; color: var(--text-muted); }
...
.job-details-col { flex: 1; min-width: 0; }
.job-company-tag {
  font-size: 0.76rem;
  font-weight: 800;
  color: var(--primary);
...
// 3. Fix initial active state in HTML: make Dashboard active by default!
html = html.replace(/<button class="nav-item" id="nav-dashboard"/, '<button class="nav-item active" id="nav-dashboard"');
html = html.replace(/<button class="nav-item active" id="nav-runner"/, '<button class="nav-item" id="nav-runner"');

html = html.replace(/<div class="page-view" id="page-dashboard"/, '<div class="page-view active" id="page-dashboard"');
html = html.replace(/<div class="page-view active" id="page-runner"/, '<div class="page-view" id="page-runner"');

// Fix Topbar initial text to match Overview
html = html.replace(/<div class="topbar-page-title" id="header-page-title">[^<]*<\/div>/, '<div class="topbar-page-title" id="header-page-title">Overview</div>');
html = html.replace(/<div class="topbar-page-sub" id="header-page-sub">[^<]*<\/div>/, '<div class="topbar-page-sub" id="header-page-sub">Real-time job intelligence & application metrics</div>');

// 4. Inject toggleThemeMode function if missing
const themeJs = `
function toggleThemeMode() {
  ...
  const btns = document.querySelectorAll('#theme-toggle-btn, .action-icon-btn[onclick="toggleThemeMode()"]');
  btns.forEach(b => b.textContent = next === 'dark' ? '☀️' : '🌙');
  if (typeof renderCharts === 'function' && typeof allJobsList !== 'undefined' && allJobsList) {
    renderCharts(allJobsList);
  }
}

// Set saved theme immediately
(function initTheme() {
  const saved = localStorage.getItem('careerforge_theme') || 'light';
  document.documentElement.setAttribute('data-theme', saved);
})();
`;

if (!html.includes('function toggleThemeMode')) {
  html = html.replace('function switchPage', themeJs + '\nfunction switchPage');
}

// 5. Write to public/index.html & dashboard/public/index.html
fs.writeFileSync(targetPath, html, 'utf8');
if (fs.existsSync(path.dirname(dashboardTargetPath))) {
  fs.writeFileSync(dashboardTargetPath, html, 'utf8');
}

console.log('✅ Navigation & Page-View isolation fixed successfully!');
